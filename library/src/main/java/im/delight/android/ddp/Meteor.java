...
public class Meteor {
    ...
    /**
     * Attempts to log in with the given login token and secret key
     *
     * @param token    the login token
     * @param listener the listener to call on success/error
     */
//	credentialSecret":"z_kTG9SFBTcjFpvpcm_DiC8DvMjVSGr47MXbtLJ1bhv
    //            credentialToken":"YCE4ggWJ0n7h7ZnEOHCxSAoJDjqqCOaAYNq3x5hVpBJpNkZX8bCJYQLu4jZzSjEB
    public void login(final String token, final String secret, final ResultListener listener) {
        final Map<String, Object> authData = new HashMap<String, Object>();
        authData.put("credentialToken", token);
        authData.put("credentialSecret", secret);
        final Map<String, Object> finalAuthData = new HashMap<String, Object>();
        finalAuthData.put("oauth",authData);
        call("login", new Object[]{finalAuthData}, listener);
    }
	 ...
	 */
    public void callWithSeed(final String methodName, final String randomSeed, ResultListener resultListener) {
